...
public class CharacterController2D : MonoBehaviour
{
	...
	private void moveVertically( ref Vector3 deltaMovement )
	{
		...
		if( (isGoingUp && !collisionState.wasGroundedLastFrame ) || pressedDown)
		...
		for( var i = 0; i < totalVerticalRays; i++ )
		{
			...
			if( _raycastHit )
			{
				...
				else
				{
					deltaMovement.y += _skinWidth;
					collisionState.below = true;
					FallOn(_raycastHit.collider.gameObject);
				}
	...
	void FallOn(GameObject collider)
	{
		if (collider.GetComponent<CharacterCollider>() != null && !collisionState.wasGroundedLastFrame && collisionState.below)
			collider.GetComponent<CharacterCollider>().FellOn(this.gameObject);
	}
